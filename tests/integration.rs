...
const VALID_12_MNEMONIC: &str = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about";
...
fn assert_success(args: &[&str]) -> String {
    ...
    assert!(ok, "Command failed: xgen {}\nstderr: {}\nstdout: {}", args.join(" "), stderr, stdout);
    stdout
}

fn assert_fails(args: &[&str]) {
    let (stdout, _stderr, ok) = xgen(args);
    assert!(!ok, "Command should have failed: xgen {}\nstdout: {}", args.join(" "), stdout);
}
...
mod chain_output {
    ...
    fn test_evm_checksummed_address() {
        let out = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0"]);
        ...
        let hex_part = &addr[2..];
        assert!(hex_part.chars().all(|c| c.is_ascii_hexdigit()), "EVM address must be hex");
    }
    ...
    fn test_bitcoin_multi_address() {
        ...
        assert!(out.matches("Address").count() >= 3, "Expected 3 Address entries");
    }
    ...
    fn test_cardano_output() {
        ...
        let addr_line = out.lines().find(|l| l.contains("Address")).unwrap();
        let addr = addr_line.split(':').nth(1).unwrap().trim();
        assert!(addr.starts_with("addr1"), "Cardano address must start with addr1");
    }
    ...
    fn test_chain_alias_ethereum() {
        ...
        assert!(out.contains("0x"), "Ethereum alias should produce 0x address");
    }
    ...
    fn test_chain_alias_telegram() {
        ...
        let addr_line = out.lines().find(|l| l.contains("Address")).unwrap();
        let addr = addr_line.split(':').nth(1).unwrap().trim();
        assert!(addr.starts_with("EQ"), "telegram alias should produce EQ address");
    }
...
mod mnemonic {
    ...
    fn test_generate_new_24_words() {
        ...
        let mnemonic_line = out.lines()
            .skip_while(|l| !l.contains("NEW MNEMONIC"))
            .skip(1)
            .next()
            .unwrap_or("");
        let word_count = mnemonic_line.split_whitespace().count();
        assert_eq!(word_count, 24, "Expected 24-word mnemonic, got {} words: {}", word_count, mnemonic_line);
    }

    #[test]
    fn test_import_valid_12_mnemonic() {
        let out = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0"]);
        assert!(out.contains("0x"), "Imported mnemonic should produce address");
        assert!(!out.contains("NEW MNEMONIC"), "Should not generate new mnemonic");
    }

    #[test]
    fn test_import_valid_24_mnemonic() {
        let out = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_24_MNEMONIC, "--index", "0"]);
        assert!(out.contains("0x"), "24-word mnemonic should produce address");
    }

    #[test]
    fn test_mnemonic_deterministic() {
        let out1 = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--json"]);
        let out2 = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--json"]);
        assert_eq!(out1, out2, "Same mnemonic + index should produce identical output");
    }

    #[test]
    fn test_mnemonic_with_passphrase() {
        let out1 = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--json"]);
        let out2 = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--passphrase", "mypass", "--json"]);
        assert_ne!(out1, out2, "Different passphrases should produce different keys");
    }

    #[test]
    fn test_invalid_mnemonic_fails() {
        assert_fails(&["gen", "--mnemonic", "this is not a valid mnemonic phrase at all", "--num", "1"]);
    }
...
mod derivation {
    ...
    fn test_specific_index() {
        let out = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "42"]);
    ...
    fn test_different_indexes_different_keys() {
        let out0 = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--json"]);
        let out1 = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "1", "--json"]);
        assert_ne!(out0, out1, "Different indexes should produce different keys");
    }
    ...
    fn test_num_without_index() {
        let out = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--num", "3"]);
        assert!(out.contains("Index 0"));
        assert!(out.contains("Index 1"));
        assert!(out.contains("Index 2"));
    }
    ...
    fn test_account_change() {
        let out = assert_success(&["gen", "--chain", "solana", "--account", "2", "--change", "1", "--num", "1"]);
    ...
    fn test_index_overrides_num() {
        ...
        assert!(out.contains("Index 3"), "With --index, only one address at that index");
    ...
    fn test_hw_sim_solana() {
        let out = assert_success(&["gen", "--chain", "solana", "--hw-sim", "--account", "1", "--num", "1"]);
...
mod output {
    ...
    #[test]
    fn test_json_output() {
        let out = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--json"]);
        ...
        assert!(out.contains("\"private_key\""), "JSON missing private_key field");
    }
    ...
    fn test_json_parseable() {
        ...
        assert_eq!(parsed["mnemonic"], VALID_12_MNEMONIC, "Mnemonic should match");
        assert!(parsed["keys"].is_array(), "Keys should be an array");
        assert_eq!(parsed["keys"].as_array().unwrap().len(), 1, "Should have 1 key");
    }

    #[test]
    fn test_json_all_chains() {
        for chain in &["evm", "btc", "solana", "ton", "doge", "xrp", "cardano", "monero"] {
            let out = assert_success(&["gen", "--chain", chain, "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--json"]);
            let parsed: serde_json::Value = serde_json::from_str(&out)
                .unwrap_or_else(|e| panic!("Invalid JSON for chain {}: {}\nOutput: {}", chain, e, out));
                assert_eq!(parsed["chain"].as_str(), Some(*chain), "Wrong chain in JSON output");
            assert!(parsed["keys"][0]["address"].is_string(), "Missing address for chain {}", chain);
        }
    }
    ...
    fn test_output_file() {
        ...
        let out = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--json", "--output", tmp]);
        assert!(out.contains("Wallet saved"), "Should show save message");
        let contents = std::fs::read_to_string(tmp).expect("File should exist");
        assert!(contents.contains("\"chain\""), "Saved file should contain JSON");
    ...
    fn test_output_file_no_json() {
        ...
        let out = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--output", tmp]);
        assert!(out.contains("Wallet saved"), "Should show save message");
        let contents = std::fs::read_to_string(tmp).expect("File should exist");
    ...
    fn test_qr_flag_does_not_crash() {
        let out = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--qr"]);
    ...
    fn test_qr_on_all_chains() {
        for chain in &["evm", "btc", "solana", "ton", "doge", "xrp", "cardano", "monero"] {
            let out = assert_success(&["gen", "--chain", chain, "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--qr"]);
            assert!(out.contains("Address"), "QR mode should show address for {}", chain);
        }
    }
}
...
mod encrypt_decrypt {
    ...
    fn test_encrypt_default_output() {
        let out = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--encrypt", "mypass"]);
        assert!(out.contains('{'), "Encrypted output should be JSON");
        assert!(out.contains("\"ciphertext\""), "Encrypted output should contain ciphertext");
        assert!(out.contains("\"salt\""), "Encrypted output should contain salt");
        assert!(out.contains("\"nonce\""), "Encrypted output should contain nonce");
    }

    #[test]
    fn test_encrypt_to_file() {
        let tmp = TempFile::new("enc_file.json");
        let out = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--encrypt", "mypass", "--output", tmp.path()]);
    ...
    fn test_encrypt_decrypt_roundtrip() {
        ...
        assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--json", "--output", dec.path()]);
        let original = std::fs::read_to_string(dec.path()).expect("Should read original");
        let _ = std::fs::remove_file(dec.path());

        assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--encrypt", password, "--output", enc.path()]);

        assert_success(&["decrypt", enc.path(), "--output", dec.path(), "--password", password]);
        let decrypted = std::fs::read_to_string(dec.path()).expect("Should read decrypted");
        assert_eq!(original, decrypted, "Decrypted content should match original");
    }

    #[test]
    fn test_encrypt_decrypt_wrong_password_fails() {
        let enc = TempFile::new("wrong_pass_enc.json");
        assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--encrypt", "correctpass", "--output", enc.path()]);
        assert_fails(&["decrypt", enc.path(), "--password", "wrongpass"]);
    }

    #[test]
    fn test_decrypt_nonexistent_file_fails() {
        assert_fails(&["decrypt", "/tmp/xgen_nonexistent_encrypted.json", "--password", "test"]);
    }
    ...
    fn test_encrypt_with_password_flag() {
        let out = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--password", "cli_pass"]);
        assert!(out.contains("\"ciphertext\""), "--password flag should encrypt output");
    }

    #[test]
    fn test_encrypt_same_data_different_ciphertext() {
        let out1 = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--encrypt", "samepass", "--json"]);
        let out2 = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--encrypt", "samepass", "--json"]);
        assert_ne!(out1, out2, "Same password + same data should produce different ciphertext (random salt/nonce)");
    }

    #[test]
    fn test_decrypt_stdout_output() {
        let enc = TempFile::new("decrypt_stdout.json");
        assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--encrypt", "pass", "--output", enc.path()]);
        let out = assert_success(&["decrypt", enc.path(), "--password", "pass"]);
        assert!(out.contains("\"chain\""), "Decrypt to stdout should output JSON");
    }

    #[test]
    fn test_encrypt_all_chains() {
        for chain in &["evm", "btc", "solana", "ton", "doge", "xrp", "cardano", "monero"] {
            let out = assert_success(&["gen", "--chain", chain, "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--encrypt", "testpass"]);
            assert!(out.contains("\"ciphertext\""), "Encrypted output for chain {} should contain ciphertext", chain);
        }
    }
}
...
mod errors {
    ...
    fn test_gen_no_args_works() {
        ...
        assert!(out.contains("NEW MNEMONIC"), "gen without args should generate new mnemonic");
        assert!(out.contains("0x"), "gen without args should produce addresses");
    }
...
mod help {
    ...
    fn test_top_level_help() {
        ...
        assert!(out.contains("decrypt"), "Help should list decrypt subcommand");
    }
    ...
    fn test_gen_help() {
        ...
        assert!(out.contains("--mnemonic"), "Gen help should list --mnemonic");
        ...
        assert!(out.contains("--passphrase"), "Gen help should list --passphrase");
        assert!(out.contains("--account"), "Gen help should list --account");
        assert!(out.contains("--change"), "Gen help should list --change");
        assert!(out.contains("--strength"), "Gen help should list --strength");
        assert!(out.contains("--password"), "Gen help should list --password");
    }

    #[test]
    fn test_decrypt_help() {
        let out = assert_success(&["decrypt", "--help"]);
        assert!(out.contains("decrypt"), "Decrypt help should mention decrypt");
        assert!(out.contains("--output"), "Decrypt help should list --output");
        assert!(out.contains("--password"), "Decrypt help should list --password");
    }
...
mod edge_cases {
    ...
    fn test_same_mnemonic_different_chains_different_keys() {
        let evm = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--json"]);
        let sol = assert_success(&["gen", "--chain", "solana", "--mnemonic", VALID_12_MNEMONIC, "--index", "0", "--json"]);
        assert_ne!(evm, sol, "Different chains should produce different outputs");
    }

    #[test]
    fn test_all_chains_same_index() {
        for chain in &["evm", "btc", "solana", "ton", "doge", "xrp", "cardano", "monero"] {
            let out = assert_success(&["gen", "--chain", chain, "--mnemonic", VALID_12_MNEMONIC, "--index", "5"]);
            assert!(out.contains("Index 5"), "Chain {} should have index 5", chain);
            assert!(out.contains("Address"), "Chain {} should have address", chain);
        }
    }

    #[test]
    fn test_large_index() {
        let out = assert_success(&["gen", "--chain", "evm", "--mnemonic", VALID_12_MNEMONIC, "--index", "9999"]);
    ...
    fn test_all_chains_multi_address() {
        for chain in &["evm", "btc", "solana", "ton", "doge", "xrp", "cardano", "monero"] {
            let out = assert_success(&["gen", "--chain", chain, "--mnemonic", VALID_12_MNEMONIC, "--num", "2"]);
    ...
    fn test_multiple_runs_different_mnemonics() {
        ...
        assert_ne!(out1, out2, "Two fresh generations should produce different mnemonics");
    }
    ...
    fn test_generated_mnemonic_preserved_in_json() {
        ...
        assert!(word_count == 12 || word_count == 24, "Generated mnemonic should be 12 or 24 words, got {}", word_count);
    }
}
